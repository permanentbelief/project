...
Page({
    ...
    onLoad: async function(options) {
        ...
        for (let i = 0; i < 7; i++) {
            var time = new Date();
            // console.log("new Date()的time是", time); // Mon Apr 11 2022 17:00:36 GMT+0800 (中国标准时间)
        ...
        console.log("获取的机构groups信息", groups);
        this.setData({
            categoryType: '居民身份证',
            genderType: '男',
            groups,
    ...
    bindChangeGroup(e) {
        ...
        db.collection('order').where({
            ...
            success: res => {
                ...
                console.log("最终得到的dateTimes时间是", dateTimes)
                this.setData({
                    dateTimes,
                    left: num - yiyueyue,
                    groupIndex: index
    ...
    async submit(e) {
        ...
        db.collection('order').add({
            data: {
                ...
                orderNum: 'ON' + new Date().getTime(), // 订单号
                type: 1, // 1核酸订单 2疫苗订单
                img: groups[groupIndex].hsImg,
                province: groups[groupIndex].province, // 省份名称
                            ...
                            if (res.confirm) {
                                ...
                                db.collection('order').doc(orderId).update({
                                    data: {
                                        ...
                                        payNum: 'PN' + new Date().getTime(), // 支付单号
                                    },
                                    success: res => {
                                        wx.showToast({
                                            title: '支付成功',
